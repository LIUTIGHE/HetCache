...
class VaceWanAttentionBlock(DiTBlock):
    ...
    def forward(self, c, x, context, t_mod, freqs):
        if self.block_id == 0:
            ...
            c = all_c.pop(-1)
        # DiTBlock.forward() returns (output, kv_cache) or just output
        result = super().forward(c, context, t_mod, freqs)
        # Handle both tuple and tensor returns
        c = result[0] if isinstance(result, tuple) else result
...
class VaceWanModel(torch.nn.Module):
    ...
    def forward(
        ...
        # print("VACE input shape:", vace_context.shape)
        # print("x (latent) shape:", x.shape)
        c = [self.vace_patch_embedding(u.unsqueeze(0)) for u in vace_context]
        c = [u.flatten(2).transpose(1, 2) for u in c]
        # print("Input feature shape:", x.shape[1])
        # print("VACE features shape:", [u.size(1) for u in c])
        c = torch.cat([
