...
class ImageSizeManager:
    ...
    def get_meshgrid(self, start, *args):
        if len(args) == 0:
            ...
            start = self._to_tuple(start)       # top-left corner   eg: 12,0
            stop = self._to_tuple(args[0])      # bottom-right corner   eg: 20,32
            num = self._to_tuple(args[1])       # target size  eg: 32,124
        else:
            raise ValueError(f"len(args) should be 0, 1 or 2, but got {len(args)}")

        grid_h = np.linspace(start[0], stop[0], num[0], endpoint=False, dtype=np.float32) # 12-20 ............32...   0-32 ............124...
    ...
    def get_2d_rotary_pos_embed(self, embed_dim, start, *args, use_real=True):
        ...
        grid = grid.reshape([2, 1, *grid.shape[1:]])   # Return a sampling matrix matching target resolution
